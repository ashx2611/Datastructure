...
void heap_op(vector<vector<e_node>> & graph, vector<rt_node> & rt, vector<h_node> &heap)
{
	...
	for (int count = 0; count < N; count++) // Find shortest path for all node
	{
		...
		for (int v = 0; v < N; v++)
		{
			// Update heap[v].cost only if is not in rt[min_index].checked, there is an edge from 
			// min to v, and total weight of path from node to  v through min_index is 
			// smaller than current value of heap[v].cost
			if ( (isValidEdge(graph, min_index, v)  && ((rt[min_index].cost + getWeightToNodeAtIndex(graph[min_index], v)) < rt[v].cost)))
			
			{
				
				rt[v].cost = (rt[min_index].cost + getWeightToNodeAtIndex(graph[min_index], v));
				rt[v].from = min_index; //graph[min_index].at(v).nb;
